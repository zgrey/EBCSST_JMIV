...
\end{point}
\begin{reply}
The decision-landscape sensitivity studies in Section~\ref{sec:experiments} (Figures~\ref{fig:010_decision} and~\ref{fig:010_decision-normalized}) supply empirical evidence addressing Type~I error and power. In the $(0\wedge 1)$ case, decisions are consistent at FTR for $r\leq 7$ and consistent at reject for $r\geq 8$ across many independent quadrature discretizations, bounding the empirical error rates near the transition. The ambiguity metric~\eqref{eq:ambiguity} and the blue-text discussion surrounding Figure~\ref{fig:010_decision} formalize this stability analysis as a decision-variability diagnostic complementary to a controlled simulated-curve study.
\end{reply}
...
\end{point}
\begin{reply}
Practical guidance for selecting $r$ (undulation dimensionality) and $n$ (quadrature nodes) has been added as blue text at the start of the decision-landscape discussion in Section~\ref{sec:experiments}. The decision-landscape framework (Figures~\ref{fig:010_decision}, \ref{fig:010_decision-normalized}, and~\ref{fig:avg_decision_land}) itself serves as the primary diagnostic: (i)~$n$ need only be large enough to visually resolve boundary features, with decisions stable for $n\in[100,500]$ thanks to the spectral convergence of the Nystr\"om quadrature (Figure~\ref{fig:nystrom_eg}); (ii)~$r$ is the more consequential parameter---the reviewer is correct that discrepancies may appear only at higher dimensions, as illustrated by the $(0\wedge 1)$ case where decisions flip at $r=8$. Rather than a raw cumulative-variance threshold, the new blue text emphasizes scanning decision variability directly via the ambiguity metric~\eqref{eq:ambiguity}, choosing $r$ beyond the ambiguity peak so that decisions are stable against the regularization level implicitly selected.
\end{reply}
...
\end{point}
\begin{reply}
We have taken several structural and editorial steps to clarify contributions and tighten the organization. First, the Contributions and Technical Outline subsection (Section~\ref{sec:intro}) has been rewritten as an explicit bulleted list in which every item is labeled \textbf{[prior]} or \textbf{[new]} and cross-references the section where it is developed. Two bullets point at \cite{grey2023separable}: one covers the polar standardization~\eqref{eq:SST} over $Gr(d,n)\times S^d_{++}$ (reviewed in Section~\ref{subsec:SST}); the other covers the separable tangent PCA algorithms (reviewed in Section~\ref{subsec:mfld_learn}). The remaining five bullets are new to this work---the Hilbert--Schmidt integral-operator interpretation of the CLO with PRRTI invariance (Section~\ref{sec:FIE}), the SRQD as a Nystr\"om-weighted polar decomposition (Sections~\ref{subsec:Nystrom}--\ref{subsec:func_approx}), cyclic Procrustes against a random asymmetric archetype (Section~\ref{subsec:cycl_Pro}), the binary-mapping $H$ formulation with De~Morgan/Bonferroni aggregation (Section~\ref{subsec:MMD}), and the decision-landscape diagnostic with the associated EBSD and lithium-ion SEM studies (Section~\ref{sec:experiments}). Two standalone paragraphs then identify the paper's primary \emph{theoretical} contribution (the RKHS lift) and primary \emph{practical} contribution (registration via finite SVDs rather than diffeomorphism optimization, enabling the ensemble-scale parameter studies).

Second, the SST background previously occupying much of Section~2 has been relocated to Appendix~\ref{secA0} (``SST Review''). The appendix has itself been rewritten as a standalone supplementary review: it retains only the derivations that do not duplicate main-body statements (Stiefel full-rank argument, the $X^{\top} = M^{\top}\widetilde{\boldsymbol{c}}$ linear-deformation derivation, and the affine-vs-polar landmark standardizations), and newly imports concise algorithmic details for the tangent PCA~\cite{grey2023separable, fletcher2003statistics, fletcher2004principal, edelman1998geometry, bendokat2020grassmann}---Fr\'echet-mean iteration, PGA SVD with the vec/vec$^{-1}$ reshape, Grassmann Exp/Log with $O(nd^2)$ complexity, SPD Exp/Log, and the representative-mismatch caveat that motivates the cyclic Procrustes step. The in-text review in Section~\ref{subsec:SST} is accordingly much shorter and now carries a dedicated paragraph on the Kendall-style scalar-dilation scale versus our generalized anisotropic-scale notion.
...
\end{point}
\begin{reply}
Our evaluation is not limited to EBSD. The Contributions \& Outline in Section~\ref{sec:intro} now explicitly promotes a non-EBSD lithium-ion battery SEM study as one of the paper's primary experimental contributions, and Section~\ref{sec:experiments} (``Segmentation Efficacy'') develops it in detail: morphological segmentation over SEM images of NMC particles, with a `goldilocks' analysis of smoothing scales illustrating the method's applicability to generic grayscale imaging.
\end{reply}
...
\end{point}
\begin{reply}
Two complementary artifacts have been added. (i)~Figure~\ref{fig:placeholder} in Section~\ref{subsec:mfld_learn} is a pipeline diagram tracing preshape curves through SRQD/polar decomposition, cyclic Procrustes registration, and separate tangent PCA on $Gr(d,n)$ and $S^d_{++}$ to the normal coordinates $(\boldsymbol{t},\boldsymbol{\ell})$ on which MMD is computed; the companion Figure~\ref{fig:SRQD} (Section~\ref{subsec:func_approx}) zooms in on the SRQD stage. (ii)~Algorithm~\ref{alg:pipeline} at the end of Section~\ref{subsec:MMD} states the full procedure as pseudocode using the manuscript notation, with step-level cross-references to Sections~\ref{subsec:preshapes}--\ref{subsec:MMD} and Truth Table~\ref{tab:logical_conjunction}.
\end{reply}
