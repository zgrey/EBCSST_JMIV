...
\end{point}
\begin{reply}
Practical guidance for selecting $r$ (undulation dimensionality) and $n$ (quadrature nodes) has been added as blue text at the start of the decision-landscape discussion in Section~4. The decision-landscape framework (Figures~10, 11, and~14) itself serves as the primary diagnostic: (i)~$n$ need only be large enough to visually resolve boundary features, with decisions stable for $n\in[100,500]$ thanks to the spectral convergence of the Nystr\"om quadrature (Figure~4); (ii)~$r$ is the more consequential parameter---the reviewer is correct that discrepancies may appear only at higher dimensions, as illustrated by the $(0\wedge 1)$ case where decisions flip at $r=8$. Rather than a raw cumulative-variance threshold, the new blue text emphasizes scanning decision variability directly via the ambiguity metric (Eq.~(25)), choosing $r$ beyond the ambiguity peak so that decisions are stable against the regularization level implicitly selected.
\end{reply}
...
\end{point}
\begin{reply}
Two complementary artifacts have been added. (i)~Figure~7 in Section~3.4 is a pipeline diagram tracing preshape curves through SRQD/polar decomposition, cyclic Procrustes registration, and separate tangent PCA on $Gr(d,n)$ and $S^d_{++}$ to the normal coordinates $(\boldsymbol{t},\boldsymbol{\ell})$ on which MMD is computed; the companion Figure~3 (Section~3.2) zooms in on the SRQD stage. (ii)~Algorithm~1 at the end of Section~3.5 states the full procedure as pseudocode using the manuscript notation, with step-level cross-references to Sections~1.5--3.5 and Truth Table~1.
\end{reply}
