...
\end{point}
\begin{reply}
The two dominant costs are cyclic Procrustes---linear in the number of curves, searching a subset of $p$-cycles per shape---and the $B=1000$ sample-permutation loop, in which each permutation recomputes $\widehat{\mathrm{MMD}}^2$ on \emph{pre-registered} coordinates so that no registration is repeated. As noted in Section~3.4, the full pipeline for the EBSD experiments in Section~4 executes in seconds on a conventional laptop for thousands of curves.
\end{reply}
