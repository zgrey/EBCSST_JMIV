...
\textcolor{red}{PLACEHOLDER [ITEM 10, R2]: CAREFULLY REVISE FOR CONCISION. CLEARLY DELINEATE WHICH RESULTS ARE FROM PRIOR WORK (G2AERO, GREY 2023) VERSUS NEW TO THIS MANUSCRIPT. THIS SHOULD ADDRESS THE OUTSTANDING TASK OF PRIOR-VS-NEW DELINEATION.}
...
Section~\ref{sec:experiments} discusses implications of all four logical decisions over the separable space using the materials EBSD data as an example. \textcolor{blue}{We also explore an application involving simple morphological segmentation applied to grayscale scanning electron microscopy (SEM) imaging of lithium-ion batteries.} Conclusions and future work follow in Section~\ref{sec:conclude}.
...
\textcolor{blue}{Varifold-based approaches~\cite{kaltenmark2017varifolds,pierson2022shape} bypass registration by comparing curves as geometric measures, and Sobolev metrics on the space of curves~\cite{bauer2017sobolev} provide principled distances with tunable regularity. However, these methods produce a single scalar distance without decomposing \emph{why} two shapes differ. In contrast, our separable framework isolates undulation from generalized scale, yielding logical explanations (Truth Table~\ref{tab:logical_conjunction}) rather than a monolithic discrepancy score.}

Our EBSD numerical experiments emphasize four scenarios where these tools deliver specific benefits: i) Figure~\ref{fig:compare_grains_111} demonstrates that the method is not suspected to be overly sensitive to statistical rejection, ii) Figure~\ref{fig:compare_grains_100} demonstrates that the method emulates conclusions consistent with human observations, iii) Figure~\ref{fig:compare_grains_010} demonstrates that the method detects differences below human-scale visual observations and, iv) Figure~\ref{fig:compare_grains_000} demonstrates that the method is capable of detecting a presumed faulty measurement. \textcolor{blue}{Our lithium-ion battery SEM experiment emphasizes a first-of-its-kind approach to solving the `goldilocks problem' of smoothing segmented boundaries before obscuring distinguishing features. This experiments facilitates a novel quantitative comparison over smoothing scales to guide segmentation hyperparameter selection.}

Our primary contribution, distinct from previous work~\cite{grey2023separable}, is a formal interpretation that discrete transformations are collocations of dual representations presented in~\cite{micheli2013matrix}. In other words, we relate novel interpretations of the SST configuration space over finite-dimensional matrix manifolds to a functional analysis over an infinite-dimensional RKHS. This interpretation facilitates a flexible choice of kernel which can be used to control the regularity of a Hilbert space of curves~\cite{micheli2013matrix} as hypothesized in any application. This motivates novel explanations and extensions between landmark configurations and the $\mathbb{R}^d$-valued RKHSs in~\cite{micheli2013matrix} representing infinite-dimensional features of curves. This is an innovation to classical configuration spaces~\cite{kendall2009shape} which have no demonstrated connection to an infinite-dimensional space of curves.
...
\textcolor{red}{PLACEHOLDER [ITEM 13, R1/R3/R4]: SST REVIEW AND ARC-LENGTH SECTIONS HAVE BEEN COPIED TO APPENDIX. REWRITE EXISTING SST DESCRIPTION AND ARC-LENGTH DESCRIPTIONS TO BE MORE CONCISE. MERGE THIS REWRITE WITH A CONCISE LITERATURE REVIEW GIVEN EXISTING REFERENCES. IMPROVE READABILITY IN SECTIONS 2--3 AS NECESSARY BUT FOCUS ON CONCISION.}
