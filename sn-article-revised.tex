...
\textcolor{blue}{Concretely, setting $\mathcal{E}\coloneqq\boldsymbol{\rho}$ and $\widehat{\mathcal{E}}\coloneqq\widehat{\boldsymbol{\rho}}$ as the arguments of $H$ in Section~\ref{sec:intro}, the MMD hypothesis test is an unsupervised, distribution-level binary classifier whose separable decomposition populates the four rows of Truth Table~\ref{tab:logical_conjunction}. Again, such a framework extends naturally to supervised or context-driven classification over labeled ensembles.}
...
\textcolor{blue}{We report timings in Table~\ref{tab:timing} using subsets of curves segmented from the battery data. The SRQD column subsumes the arc-length reparameterization it invokes internally. The pMMD two-sample test is evaluated on an even split ($N/2$ versus $N/2$) with $B = 1000$ bootstrap permutations. Each stage is reported in two configurations. The \emph{sequential} single-threaded reference exposes clean scaling laws on the algorithm itself. The \emph{parallel} configuration follows the default Python implementation, namely multiprocessing for SRQD's per-curve work and Numba's parallel JIT for the pMMD permutation loop. Measurements are taken using a Python implementation at $n=500$ landmarks and $r=50$ reduced dimensions, run on a $16$-core / $32$-thread x86-64 desktop CPU (up to $4.3$\,GHz boost) with $64$\,GB of system memory under Windows~11 and Python~3.13.}

\begin{table}
    \centering
    \begin{tabular}{r|rrrr}
        $N$ & Cyclic Pro. & SRQD & tPCA & pMMD \\
            & seq.\,/\,par. & seq.\,/\,par. & seq.\,/\,par. & seq.\,/\,par. \\
        \hline
         100 & 0.21\,/\,0.20 & 0.53\,/\,3.28 & 0.02\,/\,0.02 &  0.06\,/\,$<$0.01 \\
         %250 & 0.52\,/\,0.52 & 1.17\,/\,3.46 & 0.06\,/\,0.06 &  0.25\,/\,0.01 \\
         500 & 1.04\,/\,1.04 & 2.35\,/\,3.87 & 0.13\,/\,0.14 &  0.91\,/\,0.02 \\
         %750 & 1.58\,/\,1.54 & 3.51\,/\,4.21 & 0.24\,/\,0.24 &  2.00\,/\,0.05 \\
        1000 & 2.06\,/\,2.08 & 4.66\,/\,4.56 & 0.33\,/\,0.33 &  3.71\,/\,0.07 \\
        %1500 & 3.12\,/\,3.12 & 7.08\,/\,5.28 & 0.51\,/\,0.54 & 15.72\,/\,0.18 \\
        2000 & 4.15\,/\,4.15 & 9.38\,/\,6.11 & 0.63\,/\,0.73 & 30.71\,/\,0.38 \\
        \hline
        $\eta_{\text{seq}}$ & 1.00 & 0.97 & 1.13 & 2.08 \\
    \end{tabular}
    \caption{Wall-clock timings (seconds) for the four principal pipeline stages as the ensemble size $N$ increases, reported side-by-side for sequential (single-threaded) and parallel (default) configurations of the Python implementation. The final row reports the empirical power-law exponent $\eta_{\text{seq}}$ fit to $time(N) \approx C\, N^{\eta}$ via ordinary least squares on $(\log N,\,\log time)$ in the sequential column. Values near unity indicate linear scaling in ensemble size. The combined planar+SRQD slope of $0.97$ reflects a small-$N$ transient at $N=100$, since refitting on $N \geq 250$ recovers $\eta = 1.00$. The parallel SRQD column carries an approximately three-second multiprocess pool initialization cost. This cost dominates the small-$N$ entries and is responsible for the column remaining nearly flat through $N=1000$. Cyclic Procrustes runs single-threaded by default in both configurations because the FFT-based implementation already has a low constant factor.}
    \label{tab:timing}
\end{table}

\textcolor{blue}{In the sequential configuration, the per-curve stages (cyclic Procrustes and SRQD) scale linearly ($\eta\approx 1$) and the pMMD bootstrap scales quadratically ($\eta\approx 2$) because each of the $B=1000$ permutations touches the pooled $N\times N$ Gaussian kernel matrix. The per-curve stages and pMMD permutations are embarrassingly parallel, which the parallel column reflects directly. Numba's parallel JIT drops the $N=2000$ pMMD entry from $30.7$\,s to $0.38$\,s and removes it from the critical path. tPCA contains an inherently sequential outer iteration because Karcher's algorithm to approximate the Fr\'echet mean is iterative. Its inner per-iteration work of $N$ independent log maps is itself batched through LAPACK's gufunc machinery, so the (typically $5$--$6$) outer iterations set the wall-clock floor. Across both configurations tPCA is among the cheapest columns at every $N$. The dominant column at large $N$ is pMMD in the sequential configuration and SRQD in the parallel configuration. Full software packages in MATLAB and Python are forthcoming.}
...
\textcolor{blue}{\paragraph{Landmark configurations as elements of a non-compact Stiefel manifold.}}
...
\textcolor{blue}{\paragraph{Linear deformations and undulation as a quotient.}}
...
\textcolor{blue}{\paragraph{Two landmark standardizations (affine vs polar).}}
...
\textcolor{blue}{\paragraph{Tangent PCA algorithmic details.}
...
\textcolor{blue}{The intrinsic maps on $Gr(d,n)$ are given with Stiefel representatives~\cite{edelman1998geometry,bendokat2020grassmann}: for a direction $\Delta \in T_{[\widetilde{X}]}Gr(d,n)$ with thin SVD $\Delta = \widehat{U}\widehat{\Sigma}\widehat{V}^{\top}$, the Grassmann exponential is
