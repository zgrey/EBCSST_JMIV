...
\title[Explainable Imaging Discrepancies with Separable Shape Ensembles]{Explainable Imaging Discrepancies with Separable Shape Ensembles}
...
Beyond this particular imaging modality, our interpretations are extensible to \textit{any imaging combined with subsequent segmentation informing a large ensemble of planar curves}. Additional examples of alternative image modalities are shown in Figure~\ref{fig:eg_other_images}.
...
We emphasize that our framework is formulated as \emph{hypothesis testing} rather than clustering or classification. While the learned feature space naturally admits distance-based clustering, hypothesis testing offers a principled binary conclusion---reject or fail to reject---with a controlled significance level, directly answering whether two ensembles are statistically discrepant. Clustering, by contrast, aggregates similar features but does not provide a binary decision with quantifiable statistical guarantees. That said, clustering over the SST feature space is a complementary and equally valid approach for exploratory analysis of shape ensembles.
...
Future work will expound on the identified relationships between finite matrix manifold representations of shape and the formal interpretations of infinite dimensional analogs detailed in~\cite{micheli2013matrix}. In short, our efforts will be focused towards: i) formalizing bounds on the approximation error of reduced dimension SSTs, and ii) infinite dimensional extensions of pMMD explainable discrepancy testing over the $\mathbb{R}^d$-valued RKHSs of~\cite{micheli2013matrix}. We also hope to improve explanations to understand which patterns of shape, in more detail, contribute to approximated discrepancies.
