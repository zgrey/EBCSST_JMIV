...
\textcolor{red}{PLACEHOLDER [ITEM 11, R1/R2/R4]: DETAIL THE MMD HYPOTHESIS-TEST PROCEDURE. SPECIFY: (1) TEST STATISTIC: BIASED ESTIMATOR OF $\text{MMD}^2$ FROM LEMMA 6 OF GRETTON ET AL. (2012), COMPUTED SEPARATELY FOR UNDULATION AND SCALE COORDINATES. (2) KERNEL CHOICE: GAUSSIAN RBF $k(\mathbf{x}, \mathbf{y}) = \exp(-\|\mathbf{x} - \mathbf{y}\|^2 / 2\sigma^2)$ WITH MEDIAN PAIRWISE DISTANCE HEURISTIC (ALREADY STATED IN SEC. 4 BUT FORMALIZE HERE). (3) SAMPLE PERMUTATION PROCEDURE: $B = 1000$ PERMUTATIONS. UNDER THE NULL, ENSEMBLE LABELS ARE EXCHANGEABLE. FOR EACH SAMPLE PERMUTATION: RANDOMLY REASSIGN THE $N + \widehat{N}$ COORDINATE VECTORS TO TWO GROUPS, RECOMPUTE $\widehat{\text{MMD}}^2$. (4) CYCLIC PROCRUSTES ADDRESSES A DISTINCT PERMUTATION OVER ROWS AND THIS REGISTRATION IS PERFORMED ONCE AGAINST THE ARCHETYPE USING THE POOLED ENSEMBLE BEFORE THE SAMPLE PERMUTATION LOOP. CYCLIC PROCRUSTES PERMUTATIONS SHUFFLE PRE-REGISTERED COORDINATES, NOT RAW CURVES. VALID BECAUSE THE ARCHETYPE IS LABEL-AGNOSTIC. REJECT AT SIGNIFICANCE LEVEL $\alpha$ IF $p \leq \alpha$.}
