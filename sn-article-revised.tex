...
Precise measurement and quantification of shape is ubiquitous in imaging science with applications spanning engineering design~\cite{Grey2017,Doronina2023}, materials science~\cite{atindama2023restoration,bachmann2010inferential,Fan2020,FAN2021116810}, medical imaging~\cite{mang2019claire,durrleman2014morphometry}, functional data analysis~\cite{tucker2013generative, srivastava2016functional}, biology~\cite{hartman2023elastic,srivastava2010shape,hagwood2013testing}, and ecology~\cite{lynch2023satellite,gonccalves2021fine}. Many of the analyses in these applications benefit from separable characterizations of shape translations (\emph{where is it?}), scale variations (\emph{how big is it?}), rotations/reflections (\emph{how is it oriented?}), and everything else that remains (\emph{how nonlinear is it?}). In several applications, a choice of invariance against one or multiple of types of similarity transforms~\cite{kendall2009shape}---among other transformations~\cite{bauer2014overview}---lead to natural definitions of \emph{shape}. 

In some cases, scientists or engineers may want to answer a simple, binary question: \emph{is this the same as that}? This naturally leads to the follow-up question: \emph{if they are different, why?} We refer to solutions of this general class of problems as \emph{explainable binary classifications}---hypothesis testing that not only renders a binary decision but also explains \emph{why} the discrepancy exists.

Target applications span diverse imaging modalities, including electron microscopy, X-ray computed tomography, satellite and optical imagery, cell microscopy/histopathology, solar UV imaging, Lagrangian averaged vorticity deviations of turbulent flows, and AI-generated imagery. For example, electron backscatter diffraction (EBSD)~\cite{schwartz2009electron} provides high-resolution segmentation of material microstructures~\cite{SAVILLE2021102118,stoudt2020location}. Processing these via denoising~\cite{atindama2023restoration} and segmentation~\cite{bachmann2010texture,FAN2021116810} yields large ensembles of planar curves. Figure~\ref{fig:eg_grains} illustrates a segmented EBSD image of deformed ice generated using MTEX~\cite{Hielscher:cg5083,Fan2020,FAN2021116810}, alongside an individual `grain boundary' extracted from the ensemble.
...
\begin{quote}
    \emph{Given two images: are the curves in one image, up to Euclidean motions, the same or different from that of another and, if they are different, are differences due to linear deformations (anisotropic scale variations) or nonlinear deformations (undulations)?}
\end{quote}
...
\noindent In this work, we present an approach to answer the proposed question as a binary response augmented by \textit{explanation through separability} and formal extensions to existing work~\cite{grey2023separable,micheli2013matrix}. Unlike standard artificial intelligence (AI) models—which rely on subjective hand-labeled training and often utilize opaque `learned features' within an ambiguous `latent space,' our framework prioritizes quantifiable explanations. This avoids the common pitfalls of AI-generated responses, which, while often articulate, are prone to fabrication or hallucination.

To formally answer the proposed question in an unsupervised manner, we define a binary mapping $H: (\mathcal{E}, \widehat{\mathcal{E}}) \mapsto \lbrace 0, 1\rbrace$, where $\mathcal{E},\widehat{\mathcal{E}}$ are ensembles of feature-encoding random tensors. In our case, $0$ rejects the null hypothesis of equality and $1$ is a failure to reject (FTR). Separability of the ensemble representations facilitates a decomposition as $H(\mathcal{E},\widehat{\mathcal{E}})\coloneqq H(\mathcal{G}_r,\widehat{\mathcal{G}}_r)\wedge H(\mathcal{P},\widehat{\mathcal{P}})$. Thus, separate marginal outcomes can be combined to logically explain the aggregate. Stochastic features constituting ensembles make the associated decisions and explanations prone to error, but the compositional framework remains logical by definition.

Here $H$ is a distribution-level binary classifier over pairs of ensembles, not an instance-level label predictor. An instance-level predictor can nonetheless be \emph{built} from $H$ via \textit{context-driven classification}: fixing one argument of $H(\cdot,\cdot)$ to a curated `trusted' ensemble implicitly assigns it a label, so sweeping the other argument over discovered ensembles labels each as same-kind-as-context or not. Fixing the trusted ensemble to curves extracted from healthy histopathology tissue, for instance, yields a healthy-vs.-not-healthy decision on each query image---a seemingly binary oversimplification that is nonetheless exactly the diagnostic question that matters in screening. Determining what a discovered ensemble globally represents would require enumerating pairwise comparisons against a full database of trusted references which may be intractable. However, in several cases, we simply need to know whether a discovered ensemble matches a specific trusted context, and $H$ delivers exactly that decision with quantifiable statistical guarantees.

Additionally, the proposed paradigm is focused towards the comparison of ensemble features \textit{between} paired sets of images. However, it is just as reasonable to: i) consider partitioning ensemble features within a single image or ii) clustering features over our novel metric space. Both the `within' statistics and clustering concepts are interesting and should be studied further. However, in this work, we dedicate our focus towards the problem at hand: comparing pairs of ensembles informed by two (or more) images.

`Difference' here is stochastic and invariant to Euclidean motions---the set of all distance-preserving rotations/reflections/translations (isometries). Randomness arises from aleatoric uncertainty in the imaged objects (Figure~\ref{fig:eg_grains}) and epistemic uncertainty in segmentation hyperparameters, so decision-making must be stochastic. A naive deterministic approach---e.g., comparing pointwise pixel differences---rejects almost surely absent a perfect registration and therefore carries no information about distributional difference.

A complementary naive approach is to summarize each shape by a handful of intuitive scalars---principal lengths, area, perimeter, sphericity, and so on~\cite{stoudt2020location, ASTM_E562, ASTM_E930, ASTM_E112}. These are useful for a stochastic problem but amount to a `lossy' projection of shape: distinct geometries can share identical hand-picked summaries, so meaningful difference can collapse in unexpected ways. Despite this, commercial materials-quality standards~\cite{kazakov2019astm} are still written around such features, and we expect the shape \textit{representations} developed below to inform improvements to those standards while extending well beyond materials science.
...
\end{enumerate}
In practice, this corresponds to \emph{statistically comparing thousands or more shapes between two ensembles informed by two images}~\cite{gretton2012kernel}.

Shape is only one important piece of the quite literal puzzles in Figures~\ref{fig:eg_grains}-\ref{fig:eg_other_images}. However, we ignore the role of textures intrinsic to the tiling---i.e., the spatial arrangement of the shapes. Instead, we treat the ensemble of shapes as independent realizations from some generating distribution which, at present, \textit{ignores their translations}. Related efforts are focused towards unifying both the topology of the spatial tiling and features of shapes in image.

\subsection{Contributions \& Outline}
The manuscript follows the \emph{decompose $\mapsto$ discretize $\mapsto$ test} arc introduced above: Section~\ref{sec:FIE} together with Sections~\ref{subsec:Nystrom}--\ref{subsec:func_approx} develops the SST decomposition; Sections~\ref{subsec:cycl_Pro}--\ref{subsec:mfld_learn} treat registration and manifold learning; Section~\ref{subsec:MMD} and Section~\ref{sec:experiments} carry out the tests. Each contribution below is labeled \textbf{[prior art]} if inherited from~\cite{grey2023separable} or \textbf{[new]} otherwise, and points at the section that develops the concept.

\begin{itemize}
    \item \textbf{[prior art]}~\cite{grey2023separable} Shape polar standardization~\eqref{eq:SST} over $Gr(d,n)\times S^d_{++}$ (Section~\ref{subsec:SST}; supplementary derivations in Appendix~\ref{secA0}).
    \item \textbf{[prior art]}~\cite{grey2023separable, fletcher2003statistics, fletcher2004principal, edelman1998geometry, bendokat2020grassmann} Separable tangent PCA algorithms for intrinsic means and principal geodesic directions on $Gr(d,n)$ and $S^d_{++}$ (reviewed in Section~\ref{subsec:mfld_learn}).
    \item \textbf{[new, Section~\ref{sec:FIE}]} Interpretation of the composite linear operator as a Hilbert-Schmidt integral operator on $\mathcal{C}_d\cap\mathcal{H}_d$, with PRRTI invariance and eigenfunctions given as scaled evaluation functionals of an $\mathbb{R}^d$-valued RKHS (Definitions~\ref{def:CLO}, \ref{def:PRRTI}; Lemmas~\ref{lemma:symm}, \ref{lemma:HS_int_opt}, \ref{lemma:CLO_invariance}; Theorem~\ref{theorem:eigfuncs}).
    \item \textbf{[new, Sections~\ref{subsec:Nystrom}--\ref{subsec:func_approx}]} Square-root quadrature decomposition (SRQD) as a Nystr\"om-weighted polar decomposition of landmark configurations (Figure~\ref{fig:SRQD}).
    \item \textbf{[new, Section~\ref{subsec:cycl_Pro}]} Cyclic Procrustes registration against a random asymmetric archetype (Theorem~\ref{thm:cycl_Procrustes}).
    \item \textbf{[new, Section~\ref{subsec:MMD}]} Formulation of two-sample testing as the binary mapping $H:(\mathcal{E},\widehat{\mathcal{E}})\mapsto\{0,1\}$ with De Morgan aggregation of marginal MMD tests (Table~\ref{tab:logical_conjunction}).
    \item \textbf{[new, Section~\ref{sec:experiments}]} Decision-landscape diagnostic, an EBSD grain-boundary study illustrating all four logical conjunctions of Table~\ref{tab:logical_conjunction}, and a lithium-ion battery SEM study addressing the `goldilocks problem' of smoothing segmented boundaries without obscuring distinguishing features.
\end{itemize}

Our primary theoretical contribution is the third bullet: relating the SST configuration space over finite-dimensional matrix manifolds to a functional analysis over an infinite-dimensional RKHS. This generalizes classical configuration-space work of Kendall et al.~\cite{kendall2009shape}---which has no demonstrated connection to an infinite-dimensional space of curves---and permits flexible kernel choice~\cite{micheli2013matrix} to control regularity. Relative to varifold and Sobolev-metric approaches~\cite{kaltenmark2017varifolds,pierson2022shape,bauer2017sobolev,hartman2023varigrad,hartman2021supervised} that return a single scalar distance, and to AI-based analysis that requires hand-labeled data mapped to opaque latent features, the separable RKHS construction isolates undulation from anisotropic scale and supplies logical explanations via Truth Table~\ref{tab:logical_conjunction}.

A primary practical contribution is computational efficiency: registration reduces to finite SVDs collocated at quadrature nodes and cyclic permutations against a single asymmetric archetype (Theorem~\ref{thm:cycl_Procrustes}), rather than optimization over diffeomorphisms as in elastic and Sobolev-metric frameworks~\cite{srivastava2010shape,bauer2017sobolev,hartman2023varigrad}. Thousands of curves can therefore be decomposed and tested in seconds on a conventional machine, making tractable the parameter sweeps of Section~\ref{sec:experiments}---including the sub-visual smoothing-induced discrepancies in Figure~\ref{fig:compare_grains_010} and the `goldilocks' segmentation analysis of Figure~\ref{fig:avg_decision_land}. To our knowledge, comparisons over such scales and sweeps have not previously been demonstrated for curve ensembles.

\subsection{Notation}
In general, bold fonts distinguish vector-valued objects from capitalized fonts which represent matrix-valued objects. We reuse common conventions for the following spaces of matrices:
\begin{itemize}
    ...
    \item $St(d,n)$ is the Stiefel, the space of $n\times d$ matrices with orthonormal columns while $\mathbb{R}^{n \times d}_*$ is the corresponding non-compact Stiefel of full rank ($d$) matrices.
...
Quotient topologies are written $\mathcal{A}/\mathcal{B}$ (``dividing out'' the action of $\mathcal{B}$), and square brackets $[\cdot]$ denote the induced equivalence classes (e.g.\ $[\boldsymbol{c}]\in\mathcal{A}/\mathcal{B}$). Elsewhere, we define new notation explicitly and use ``$\coloneqq$'' to indicate identification by definition. Angle brackets $\langle\cdot,\cdot\rangle$ denote bilinear inner products; the scalar dot product is written equivalently as $\boldsymbol{a}^{\top}\boldsymbol{b}=\boldsymbol{a}\cdot\boldsymbol{b}$; vectors are always tall. One conflict arises with a redundant use of $\alpha$. The bold notation $\boldsymbol{\alpha}$ is a vector of constant coefficients while $\alpha$ defines the significance level of the hypothesis tests.

\subsection{Review} \label{subsec:SST}
Related work~\cite{grey2023separable, bryner20142d, srivastava2016functional, kendall2009shape} defines finite representations of curve data as \textit{configuration spaces}. Following the development of SST~\cite{grey2023separable} for aerodynamic shape representations~\cite{doronina2022grassmannian}, we let $X = (\boldsymbol{x}_1,\dots,\boldsymbol{x}_n)^{\top}$ be $n$ unique \emph{landmarks} drawn from a curve $\boldsymbol{c}:\mathcal{I} \subset \mathbb{R} \rightarrow \mathbb{R}^d$ via
\begin{equation}\label{eq:data}
    \boldsymbol{x}_i = \boldsymbol{c}(s_i), \quad i=1,\dots,n,
\end{equation}
with $X\in\mathbb{R}^{n\times d}_*$ full-rank to exclude degenerate points/lines~\cite{absil2008optimization, grey2023separable}. Appendix~\ref{secA0} offers a more extensive review.

The central object is the thin SVD of the centered landmarks,
\begin{equation} \label{eq:landmark_stnd}
    (X - B(X))^{\top} \,\,\overset{SVD}{=}\,\, U\Sigma V^{\top},
\end{equation}
with $B(X) = (1/n)\boldsymbol{1}_{n,n}X$. The polar standardization~\cite{grey2023separable} yields $X = \widetilde{X}P$ with $\widetilde{X} \coloneqq VU^{\top}$ and $P \coloneqq U\Sigma U^{\top}\in S^d_{++}$. Identifying $S^d_{++} \cong GL_+(d,\mathbb{R})/SO(d)$ via~\cite{fletcher2003statistics} and $Gr(d,n)\cong \mathbb{R}^{n\times d}_*/GL(d,\mathbb{R})$ via~\cite{absil2008optimization}, the equivalence class $[\widetilde{X}]\in Gr(d,n)$ captures nonlinear \emph{undulation} while $P\in S^d_{++}$ captures generalized anisotropic \emph{scale}, independent of rotations and reflections. The pair $([\widetilde{X}],P)\in Gr(d,n)\times S^d_{++}$ parametrizes the SST
\begin{equation}\label{eq:SST}
    X(\boldsymbol{t},\boldsymbol{\ell}) = \widetilde{X}(\boldsymbol{t})P(\boldsymbol{\ell}),
\end{equation}
with $\boldsymbol{t} \in \mathbb{R}^r$ ($1< r < d(n-d)$) and $\boldsymbol{\ell}\in \mathbb{R}^3$ learned by separate tangent PCA~\cite{fletcher2004principal, edelman1998geometry, grey2023separable} over the aggregate ensemble.

A key distinction from classical configuration-space analyses~\cite{kendall2009shape,srivastava2010shape,srivastava2016functional} is the generalized notion of scale. Classical treatments model scale as a scalar dilation, whereas an SST takes the full cone $S^d_{++}$ of anisotropic deformations, so the product $Gr(d,n)\times S^d_{++}$ separates undulation from anisotropic scale---a separation essential to the hypothesis tests of Section~\ref{subsec:MMD}.
...
As before, preshapes are defined as open or closed curves $\boldsymbol{c}:\mathcal{I} \rightarrow \mathbb{R}^d$ assumed to be immersions and/or embeddings~\cite{micheli2013matrix,bauer2014overview,srivastava2010shape,mumford2006riemannian,younes2008metric,hartman2023elastic}---e.g., for $d=2$, $\boldsymbol{c}$ in our application is a planar immersion over $\mathcal{I}$ as the unit circle or equivalent compact, connected $1$-manifold denoted $\text{Imm}(\mathcal{I}, \mathbb{R})$. Applications are benefited by representations of curves over regularized spaces which control noisy measurement oscillations, e.g., those with increasing Lipschitz constant of curves $\mathcal{C}_d(\lambda_r) \subseteq \mathcal{C}_d(\lambda_{r+1})$ such that $0<\lambda_r \leq \lambda_{r+1}<\infty$ where
        \begin{multline}\label{eq:reg_Hilbert_space}
             \mathcal{C}_d(\lambda_r) \coloneqq
             \lbrace \boldsymbol{c} \in \text{Imm}(\mathcal{I},\mathbb{R}^d) \,:\,\\ \Vert \boldsymbol{c}(u) - \boldsymbol{c}(s)\Vert \leq \lambda_r\vert u - s\vert\rbrace,
        \end{multline}
...
Sobolev-type metrics are standard for controlling regularity and avoiding vanishing distances over (manifold) topologies of preshapes and shapes~\cite{michor2004vanishing,bauer2014overview,micheli2013matrix}, but $\mathcal{C}_d(\lambda_r)$ with the $2$-norm and weak differentiability suffices here because the subsequent analysis projects onto \emph{finite} matrix manifolds on which distances do not vanish~\cite{bauer2014overview}. Although the space is naturally parametrized by the Lipschitz bounding constant, we do not pursue that formalism in this work and refer simply to $\mathcal{C}_d$ with an unknown finite bound; we nonetheless observe empirically in Section~\ref{subsec:mfld_learn} that the learned submanifold dimensionality $r$ respects the nesting $\mathcal{C}_d(\lambda_r)\subseteq\mathcal{C}_d(\lambda_{r+1})$.
...
\begin{proof}
    ...
    \end{align}
\end{proof}
Thus, the CLO and the associated integral operation with Euclidean invariance of the arc-length measure fall into the class of PRRTI operators (Definition~\ref{def:PRRTI}), also referred to as TRI kernels~\cite{micheli2013matrix}. Lemma~\ref{lemma:CLO_invariance} implies the angle between pairs of landmarks, $\theta$, is preserved under \emph{Euclidean transforms} according to~\eqref{eq:inner_prod_ident}---i.e., angles between landmarks are unchanged by distance-preserving transformations (isometries). Thus, we can interpret the FIE with the HS integral operator~\eqref{eq:HS_operator} as a separation of general linear (scale) deformations and Euclidean invariant angles between discrete pairs of landmarks (undulations). 
...
\begin{definition}[Evaluation functionals of RKHS,~\cite{micheli2013matrix}] \label{def:eval_func}
    ...
    $$
    v:\mathcal{H}_d \rightarrow \mathbb{R}\,:\,\boldsymbol{\tau} \mapsto \boldsymbol{\alpha} \cdot \boldsymbol{\tau}(s),
    $$
    are linear and continuous over $\mathcal{H}_d$ and in $\boldsymbol{\alpha}\in \mathbb{R}^d$ for all $s \in \mathcal{I}$, i.e., $v\in \mathcal{H}^*_d$ is a dual representation of the curve.
\end{definition}
...
\begin{figure}
    \centering
    \includegraphics[width=0.55\linewidth]{img/PL_SSTs-1.pdf}
    \hspace*{-1.65em}\includegraphics[width=0.95\linewidth]{img/PL_SSTs-2.pdf}
    \caption{(top) Piecewise linear polygonal shapes with identical generalized scales and increasing number of faces from triangle to octagon. (bottom) Corresponding orthonormal eigenfunctions. Solid lines correspond to $j=1$ while dashed lines are $j=2$ and colors match the corresponding shape above. The eigenfunctions are distinct and, thus, each polygon is distinct but appear to naturally converge to the eigenfunctions of the circle (black curve/functions) with an increasing number of faces.}
    \label{fig:PL_SST}
\end{figure}
...
\begin{equation}
    ...
    \end{matrix}\right),
\end{equation}
we obtain phase shifted eigenfunctions, $\widetilde{v}_1(s) = \pm\sqrt{2}\cos(2\pi s + \theta)$ and $\widetilde{v}_2(s) = \pm \sqrt{2}\sin(2\pi s + \theta)$, but (up to shifted reparametrization) our conclusion persists---all three of these curves are equally undulating with the only distinction being linear scale variations according to $\sigma_1^2 = a^2/2$ and $\sigma_2^2 = b^2/2$. In contrast, examples of piecewise linear polygons with increasing face counts admitting distinct undulations but identical anisotropic scales is shown in Fig.~\ref{fig:PL_SST}.
...
Based on these conclusions, orthonormal (standardized) parametric forms satisfying $\Vert \boldsymbol{v} \Vert_{\mathcal{H}_d} = 1$ represented by dual evaluation functionals with (bounded) \emph{periodic shift invariant kernel} equivalently constitute \emph{expansions of undulation}. We may refer to such an expansion as a \emph{Mercer series of undulation} over the construction of a dual space of PRRTI-features to approximate the tuple of shape eigenfunctions, $\widetilde{\boldsymbol{v}} \coloneqq (\widetilde{v}_1,\dots,\widetilde{v}_d)^{\top}$, with any desired regularity.
...
Having formalized the SST decomposition and its RKHS interpretation in Section~\ref{sec:FIE}, we now turn to the numerical machinery that instantiates it and progressively moves from the \emph{decompose} stage into \emph{discretize} and on to \emph{test}. The developments below assume an upstream image-segmentation step as preprocessing has produced sets of ordered landmarks or integrable curves; we do not assume nor prescribe segmentation types, and related registration/alignment literature~\cite{dogan2015fast, al2013continuous, rangarajan1997softassign, srivastava2016functional} is complementary.

%\input{sec/sub_Nystrom}

\subsection{Nystr{\"o}m's Method for Curves}\label{subsec:Nystrom}
With explicit knowledge of the curve's construction, we may be able to compute eigenfunctions exactly. However, \emph{absent explicit definitions of curves} and for convenience in exploring a variety of sufficiently smooth (regular) interpolations and approximations, we elaborate on a spectral method as a re-weighting of the SVD~\eqref{eq:landmark_stnd} for approximating eigenfunctions.
...
\begin{figure*}
    \centering
    \begin{tikzpicture}[
        % Create a style to keep the code clean
        block/.style={
            draw, 
            minimum width=#1, 
            minimum height=15mm, 
            text height=2ex, % Adjusts the space above the baseline
            text depth=1.25ex, % Adjusts the space below the baseline
            anchor=north west
                }
            ]
        % Node X (The reference point)
        \node[block=25mm, label=left:$d$, label=above:$n$, label=below:$T^{\top}_n$] (X) 
            {$\cdot\,\Biggl\vert\,\cdot\, \boldsymbol{\tau}(s_i)\cdot\,\Biggl\vert\,\cdot$};
        
        % Node W: We override the height to 20mm here.
        \node[draw, minimum width=25mm, minimum height=25mm, label=above:$n$, label=below:$\sqrt{W}$, anchor=north west] (W) 
            at ([xshift=2mm]X.north east) {};
        \draw[-] (W.north west) -- (W.south east);
        
        % Node U
        \node[block=15mm, label=left:$d$, label=$d$, label=below:$A$] (U) 
            at ([xshift=8mm]W.north east) {$\begin{matrix} *\quad & *\\ *\quad & * \end{matrix}$};
        
        % Node Sigma
        \node[block=15mm, label=$d$, label=below:$\Sigma_d$] (Sigma) 
            at ([xshift=2mm]U.north east) {};
        \draw[-] (Sigma.north west) -- (Sigma.south east);
        
        % Node V
        \node[block=25mm, label=$n$, label=below:$\widetilde{V}^{\top}$] (V) 
            at ([xshift=2mm]Sigma.north east) {$\cdot\,\Biggl\vert\,\cdot\, \widetilde{\boldsymbol{v}}(s_i)\,\cdot\Biggl\vert\,\cdot$};
        
        % Equals sign
        \path (W.east) -- node {$=\quad$} (U.west);
    
    \end{tikzpicture}

    \caption{The SRQD of a translated (centered) curve where $W$ is a diagonal of quadrature weights $w_i$, $\boldsymbol{\tau}(s_i)$ are centered landmarks evaluated at quadrature nodes, and $\widetilde{\boldsymbol{v}}(s_i)$ are the $d$-tuples of evaluation functionals at quadrature nodes.}
    \label{fig:SRQD}
\end{figure*}
\begin{figure}
    \centering
...
\begin{equation} \label{eq:SRQD}
    T_n^{\top}\sqrt{W}\,\,\overset{SVD}{=}\, A\Sigma_d\widetilde{V}^{\top},
\end{equation}
for all curves in the ensemble utilizing rapid rank-$d$ decomposition schemes. A visual add is given in Figure~\ref{fig:SRQD}.
...
\begin{align} \label{eq:derived_polar_decomp}
    \sqrt{W}T_n &= \widetilde{V} \Sigma_dA^{\top}\\ \nonumber
...
\begin{figure*}
    ...
    \caption{(left) An arbitrary cyclic permutation and rotation of a uniformly discrete grain shape with colors indicating row index, $n=500$. (center) An orthogonal Procrustes match. (right) A cyclic Procrustes match.}
...
\begin{proof}
    ...
    \end{matrix}
    $$
    However, distinct from~\cite{golub1973differentiation}, the bijection, $$\pi_{\mathcal{U}}(\cdot) \coloneqq (\cdot)(V_p\Omega_p^{-1}V_p^{\top}),$$ is defined by the SVD, $\mathcal{L}(p) \,\overset{SVD}{=} U_p\Omega_pV_p^{\top}$, such that $\Omega_p = \text{diag}(\omega_1(p),\omega_2(p),\dots,\omega_d(p))$ with $\omega_1(p) \geq \omega_2(p)\geq \dots \geq \omega_d(p)>0$ and $U_p,V_p \in O(d)$. Utilizing the definition of $\pi_{\mathcal{U}}$, notice
    \begin{align*}
        ...
        &= \Vert \mathcal{L}(p) \Vert_1 \quad \tag{$\star$}
    \end{align*}
    and 
    \[
        \pi_{\mathcal{U}}(\mathcal{L}(p)) = U_pV_p^{\top}.    \tag{$\star\star$}
    \]
    Next, we argue the (extraneous) parametric constraint, $R = \pi_{\mathcal{U}}(\mathcal{L}(p))$ for any $p$, is best. By definition, $\pi_{\mathcal{U}}(\mathcal{L}(p))$ is best if and only if $\langle \pi_{\mathcal{U}}(\mathcal{L}(p)),\mathcal{L}(p)\rangle_F > \langle R ,\mathcal{L}(p)\rangle_F$ for all $R \neq \pi_{\mathcal{U}}(\mathcal{L}(p)) \in O(d)$ and $p \in \mathcal{N}$. Equivalently, it is sufficient to show 
    $$\langle R - \pi_{\mathcal{U}}(\mathcal{L}(p)), \mathcal{L}(p) \rangle_F<0
    $$ 
    for all $R \neq \pi_{\mathcal{U}}(\mathcal{L}(p)) \in O(d)$. Given $(\star)$,
    \begin{align*}
    ...
    \end{align*}
    Then, by the original argument of orthogonal Procrustes solution~\cite{gibson1962least, schonemann1966generalized, lawrence2019purely}, 
    \begin{align*}
        \langle R,\mathcal{L}(p)\rangle_F &= \text{tr}(R^{\top}U_p\Omega_pV_p^{\top}) \\
        &= \text{tr}(V_p^{\top}R^{\top}U_p\Omega_p) \\
        &= \langle Q_p, \Omega_p\rangle_F
    \end{align*}
    for some $Q_p =  U_p^{\top}RV_p \in O(d)$ which satisfies $\langle Q_p, \Omega_p\rangle_F \leq \langle \mathbb{I}_d, \Omega_p\rangle_F = \Vert \mathcal{L}(p) \Vert_1$ with equality when $Q_p = \mathbb{I}_d$. However, $Q_p = \mathbb{I}_d$ if and only if $R = U_pV_p^{\top}$ and thus, by $(\star\star)$, $R = \pi_{\mathcal{U}}(\mathcal{L}(p))$. Hence, $\langle R,\mathcal{L}(p)\rangle_F - \Vert \mathcal{L}(p) \Vert_1 < 0$ for all $R \neq \pi_{\mathcal{U}}(\mathcal{L}(p)) \in O(d)$.
\end{proof}

Theorem~\ref{thm:cycl_Procrustes} aligns each segmented curve to a fixed `archetype' $Y$ collocated at the same quadrature nodes and implicitly defines a consistent starting landmark (a `best' discrete phase shift) over the rows of $X$. Uniqueness requires the archetype to be asymmetric\footnote{A `featureless' circle produces a constant objective.}; in practice, a random draw from the pooled ensemble suffices, and we normalize $X$ and $Y$ to unit length (dilating $Y$ to match the scale of $X$) for numerical stability. Fig.~\ref{fig:cyc_Procrustes} shows registration to within machine precision under an arbitrary rotation and cyclic permutation, and related continuous treatments~\cite{dogan2015fast} accelerate alignment via fast Fourier transforms. We note that cyclic Procrustes itself induces a modified Procrustean metric~\cite{srivastava2016functional} over discrete shapes and could serve as a standalone discrepancy measure on shape ensembles, independent of the SST and MMD machinery developed in the remainder of this work.

A natural alternative is to align against a `central shape' such as the Fr\'echet mean. However, the Fr\'echet mean is coupled with alignment---alignment needs an archetype, the mean needs aligned data---and the iterative scheme (align $\mapsto$ mean $\mapsto$ repeat) may or may not converge for the discrete cyclic permutation. Theorem~\ref{thm:cycl_Procrustes} instead guarantees an optimal alignment to any template, so the key requirement is asymmetry, not Fr\'echet optimality. Figures~\ref{fig:010_decision}--\ref{fig:010_decision-normalized} confirm empirically that the random-archetype registration is stable across $n$ and, somewhat remarkably, that coordinate normalization alone stabilizes decisions even under random cyclic permutations.
...
\begin{figure*}
    ...
    \caption{The diagram depicts various named spaces and essential transformations to map from translated curves to normal coordinates over a section of the product submanifold. Here, $\pi$ represents a normal projection of the first factor onto the equivalence classes of the Grassmannian. Dashed arrows indicate mappings into discrete representatives.}
    \label{fig:placeholder}
\end{figure*}
With an ensemble of accurate, discrete, cyclically-aligned, preshape PRRTI-features $\lbrace \widetilde{X}\rbrace$ and their corresponding generalized scales $\lbrace P\rbrace$ now in hand, we pivot from constructing these matrices to learning with thousands of them informed by thousands of curves spanning the pair of images. Supplemental algorithmic details appear in Appendix~\ref{secA0}.
...
This formal problem statement is the quantitative analog of the proposed explainable binary classification: \textit{test if distributions of random shape coordinates partitioned between two images are discrepant, modulo Euclidean actions, and whether those discrepancies result from distinct distributions over undulation, $\lbrace \boldsymbol{t} \rbrace$ from the first image versus $\lbrace \widehat{\boldsymbol{t}} \rbrace$ from the second, \emph{or} generalized scale, $\lbrace \boldsymbol{\ell} \rbrace$ from the first image versus $\lbrace \widehat{\boldsymbol{\ell}} \rbrace$ from the second.}

Instead of testing for equivalence, we look for statistically significant differences. This complementary separable formalism is simply De Morgan's theorem, $(\rho_{\boldsymbol{t}} = \widehat{\rho}_{\boldsymbol{t}})\wedge (\rho_{\boldsymbol{\ell}} = \widehat{\rho}_{\boldsymbol{\ell}}) \leftrightarrow \neg ((\rho_{\boldsymbol{t}} \neq \widehat{\rho}_{\boldsymbol{t}}) \vee (\rho_{\boldsymbol{\ell}} \neq \widehat{\rho}_{\boldsymbol{\ell}}))$. Truth Table~\ref{tab:logical_conjunction} details this formalism where \textit{logical truth} corresponds to \textit{failure to reject} (FTR) the null hypothesis of equivalence, $(\rho_{(\cdot)} = \widehat{\rho}_{(\cdot)})$, when comparing against approximated thresholds as discussed in~\cite{gretton2012kernel}.

Concretely, setting $\mathcal{E}\coloneqq\boldsymbol{\rho}$ and $\widehat{\mathcal{E}}\coloneqq\widehat{\boldsymbol{\rho}}$ as the arguments of $H$ in Section~\ref{sec:intro}, the MMD hypothesis test is an unsupervised, distribution-level binary classifier whose separable decomposition populates the four rows of Truth Table~\ref{tab:logical_conjunction}. Again, such a framework extends naturally to supervised or context-driven classification over labeled ensembles.

Because the separable test is a logical conjunction of two marginal MMD tests, a union bound gives joint Type~I error at most $2\alpha$ under the intersection null. We therefore adopt the Bonferroni correction and run each marginal at $\alpha/2$ so that the joint level is bounded by $\alpha$. For Section~\ref{sec:experiments} at $\alpha=0.02$ (hence $\alpha/2=0.01$), all rejections satisfy $p\ll 0.01$ and all FTRs satisfy $p\gg 0.02$. The De~Morgan identity underlying Truth Table~\ref{tab:logical_conjunction} is a logical formalism for aggregating results while Bonferroni is statistical correction on the aggregated results.
...
Separability does not assume nor imply statistical independence of $(\boldsymbol{t},\boldsymbol{\ell})$. Distinct normal coordinates describe distinct shape features. The separable decomposition in Figure~\ref{fig:low_dim_grain} visualizes this distinction. Varying $\boldsymbol{t}$ alone modulates boundary roughness (curve regularity) while varying $\boldsymbol{\ell}$ alone modulates anisotropic size. However, physical processes---e.g.\ recrystallization correlations driving both grain growth and boundary roughness---can couple them. A straightforward (un-pursued) diagnostic is to test whether the joint factors as a product of marginals via $\text{MMD}[\mathcal{F},\boldsymbol{\rho},\rho_{\boldsymbol{t}}\rho_{\boldsymbol{\ell}}\,]$~\cite{gretton2012kernel} (the Hilbert--Schmidt information criterion). Whether or not the joint factors, the product metric~\eqref{eq:pMMD} remains valid and the aggregate conclusion of Truth Table~\ref{tab:logical_conjunction} still holds. Decomposition simply determines whether direct comparison of the joint distributions would add explanatory content beyond the marginal tests.

The test statistic is the biased empirical estimator of~\cite{gretton2012kernel}, computed separately for undulation and scale coordinates, with $\mathcal{F}$ realized as the RKHS with the (characteristic) Gaussian kernel under the standard median pairwise-distance heuristic.

Two permutation operations enter the workflow. Cyclic Procrustes permutation (Theorem~\ref{thm:cycl_Procrustes}) acts on the rows of each landmark matrix to register every shape against the random asymmetric archetype, once and prior to testing. Sample permutations act on the ensemble labels. The $N+\widehat{N}$ pre-registered coordinate vectors for either ensemble are pooled and randomly reassigned to two groups of sizes $N,\widehat{N}$, then MMD is recomputed. This ensemble-label resampling is called \textit{bootstrapping}. The permutation $p$-value is rejected at significance $\alpha$ when it is below $\alpha/2$ per the Bonferroni adjustment.

In summary, given ensembles of segmented curve features as data from a pair of images, separate hypothesis tests over one image $\lbrace \boldsymbol{t} \rbrace \sim \rho_{\boldsymbol{t}}$ versus the other $\lbrace \widehat{\boldsymbol{t}}\rbrace \sim \widehat{\rho}_{\boldsymbol{t}}$, and likewise $\lbrace \boldsymbol{\ell} \rbrace \sim \rho_{\boldsymbol{\ell}}$ versus $\lbrace \widehat{\boldsymbol{\ell}}\rbrace \sim \widehat{\rho}_{\boldsymbol{\ell}}$, offer logical tests to \emph{explain} differences aggregated with the product maximum mean discrepancy (pMMD). Numerical examples are depicted and described in Figures~\ref{fig:compare_grains_111}-\ref{fig:compare_grains_000} emphasizing each row of the logical conjunction shown in Truth Table~\ref{tab:logical_conjunction}. Algorithm~\ref{alg:pipeline} collects the full workflow.
...
\begin{enumerate}[label=\roman*)]
    \item $(1\wedge 1)$, Figure~\ref{fig:compare_grains_111}: no over-sensitivity to statistical rejection---no useful discrepancy in undulation or scale.
    \item $(1\wedge 0)$, Figure~\ref{fig:compare_grains_100}: conclusions consistent with human observation---visible grain-size differences yield a scale-only rejection.
    \item $(0\wedge 1)$, Figure~\ref{fig:compare_grains_010}: sub-visual discrepancy detection---boundary smoothing, a preprocessing step ubiquitous in EBSD (and in image-segmentation pipelines more broadly), induces undulation-only differences invisible to cursory human inspection that are nonetheless flagged as a statistically significant rejection. This sensitivity is among the method's most compelling demonstrations.
    \item $(0\wedge 0)$, Figure~\ref{fig:compare_grains_000}: detection of a faulty measurement---an out-of-focus instrument is flagged by simultaneous rejection on both factors.
\end{enumerate}

The $(0\wedge 1)$ case of Figure~\ref{fig:compare_grains_010} was designed by taking case $(1\wedge 1)$ and processing the same boundaries with and without smoothing; Figure~\ref{fig:101_zoomed_compare} magnifies the difference. The null is rejected at $\alpha = 0.02$ despite the discrepancy being largely invisible to cursory inspection, which makes the method useful for \textit{detecting the presence of any preprocessing or synthetic generation of data} that hand-picked features would miss.

\subsection{Decision Landscapes}
The explainable binary classification in all cases is predicated on the selected shape dimensionality, $r$, number of quadrature nodes, $n$, and kernel heuristics like scale parameter selection and coordinate normalization. However, given the computational efficiency of the method, we can easily motivate parameter studies to understand the \textit{decision landscape} over either set of separate coordinates. That is, decision landscapes are the primary diagnostic for selecting the quadrature size $n$ and the undulation rank $r$. We focus on determining $r$ and $n$ empirically in the case $(0 \wedge 1)$ for decisions over Grassmannian undulations, $\text{MMD}(\mathcal{H},\rho_{\boldsymbol{t}},\widehat{\rho}_{\boldsymbol{t}}) $.

Spectral convergence of Nystr\"om quadrature on smooth closed curves means a modest $n$ suffices; Figures~\ref{fig:010_decision} and~\ref{fig:010_decision-normalized} confirm decisions are stable across $n\in[100,500]$, so $n$ should simply be large enough to approximate PRRTI features of the worst-case boundary in the ensemble. The rank $r$ is more consequential but should \emph{not} be chosen to maximize variance coverage as in a traditional PCA. Smaller $r$ acts as a \textit{dimension reduction} that regularizes shapes against noisy undulations (Figure~\ref{fig:low_dim_grain}), which is often desirable~\cite{doronina2025aerodynamic}. We instead study decision variability directly via the \emph{ambiguity}~\eqref{eq:ambiguity}---a scaled Bernoulli variation of the rejection proportion conditioned on $r$---and choose $r$ with limited ambiguity so that decisions are stable against the chosen regularization level.
...
Given the changing decision landscape, it is helpful to introduce \textit{ambiguity} as scaled Bernoulli variance,
...
\begin{figure*}
    \centering
    \includegraphics[width=0.495\linewidth]{img/new_bat.pdf}
    \includegraphics[width=0.495\linewidth]{img/old_bat.pdf}
...
The global trends between the two decision landscapes are distinct but the ability to circumvent row-wise registrations by simply normalizing coordinates could offer massive computational advantages---i.e., \textit{optimal alignments may be less important than previously hypothesized if utilizing coordinate normalization}. In practice, decision landscapes should be utilized to understand sensitivities to decision making in any replicated or extended work.
...
\subsection{Computational Burdens}
We report timings in Table~\ref{tab:timing} using subsets of curves segmented from the battery data. The SRQD column subsumes the arc-length reparameterization and cyclic Procrustes alignment it invokes internally. The pMMD two-sample test is evaluated on an even split ($N/2$ versus $N/2$) with $B = 1000$ bootstrap permutations. Two of the three stages are reported in two configurations. The \emph{sequential} single-threaded reference exposes clean scaling laws on the algorithm itself. The \emph{parallel} configuration follows the default Python implementation, namely multiprocessing for SRQD's per-curve work and Numba's parallel JIT for the pMMD permutation loop. Measurements are taken using a Python implementation at $n=500$ landmarks and $r=50$ reduced dimensions, run on a $16$-core / $32$-thread x86-64 desktop CPU (up to $4.3$\,GHz boost) with $64$\,GB of system memory under Windows~11 and Python~3.13.

\begin{table}
    \centering
    \begin{tabular}{r|cccc}
        $N$ &  SRQD & tPCA & pMMD \\
        (total) & (seq.\,/\,par.) & (seq.\,/\,par.) & (seq.\,/\,par.) \\
        \hline
         100 & \textbf{0.43}\,/\,2.29 & 0.03\,/\,-- &  0.07\,/\,$<$\textbf{0.01} \\
         %250 & 0.52\,/\,0.52 & 1.17\,/\,3.46 & 0.06\,/\,0.06 &  0.25\,/\,0.01 \\
         500 & \textbf{1.93}\,/\,2.43 & 0.15\,/\,-- &  0.93\,/\,\textbf{0.03} \\
         %750 & 1.58\,/\,1.54 & 3.51\,/\,4.21 & 0.24\,/\,0.24 &  2.00\,/\,0.05 \\
        1000 & 3.74\,/\,\textbf{2.52} & 0.36\,/\,-- &  4.92\,/\,\textbf{0.07} \\
        %1500 & 3.12\,/\,3.12 & 7.08\,/\,5.28 & 0.51\,/\,0.54 & 15.72\,/\,0.18 \\
        2000 & 7.41\,/\,\textbf{3.01} & 0.66\,/\,-- & 31.91\,/\,\textbf{0.36} \\
        \hline
        $\eta_{\text{seq}}$ & 0.96\,/\, 0.08 & 1.10\,/\, -- & 2.09 \,/\, 1.61 \\
    \end{tabular}
    \caption{Wall-clock timings (seconds) for the principal pipeline stages as the ensemble size $N$ increases, reported side-by-side for sequential (seq., single-threaded) and parallel (par.) configurations of a Python implementation. The SRQD column subsumes the first three steps of Algorithm~\ref{alg:pipeline} and the tPCA column does not parallelize. The final row reports the empirical power-law exponent $\eta_{\text{seq}}$ fit to $time(N) \approx C\, N^{\eta}$ via ordinary least squares. Values near unity indicate linear scaling in ensemble size.}
    \label{tab:timing}
\end{table}

In the sequential configuration, the aggregated per-curve stages labeled `SRQD' (planar reparametrization, cyclic Procrustes, and SRQD) scale linearly ($\eta\approx 1$) and the pMMD bootstrap scales quadratically ($\eta\approx 2$) because each of the $B=1000$ permutations touches the pooled $N\times N$ Gaussian kernel matrix. The per-curve stages and pMMD permutations are embarrassingly parallel. The sequential SRQD slope of $0.96$ reflects a small-$N$ transient at $N=100$, and refitting on $N \geq 250$ (not shown) recovers $\eta = 1.00$. The parallel SRQD column carries an approximately three-second multiprocess pool initialization cost. This cost dominates the small-$N$ entries and is responsible for the column remaining nearly flat through $N=1000$. Among the per-curve stages aggregated into the SRQD column, cyclic Procrustes contributes a low constant factor courtesy of its FFT-based implementation, and is not separately accelerated.

Numba's parallel JIT drops the $N=2000$ pMMD entry from $31.9$\,s to $0.36$\,s and removes it from the critical path. The tPCA step contains an inherently sequential outer iteration because Karcher's algorithm to approximate the Fr\'echet mean is iterative---details in Appendix~\ref{secA0} and~\cite{grey2023separable}. However, across both configurations tPCA is among the cheapest columns at every $N$. The dominant column at large $N$ is pMMD in the sequential configuration and SRQD in the parallel configuration. Full software packages in MATLAB and Python are forthcoming.

\section{Conclusion}\label{sec:conclude}
We established a formal connection between separable shape tensors and the dual evaluation functionals of~\cite{micheli2013matrix}, developed a spectral (Nystr\"om) approximation of SSTs that is accurate for sufficiently regular curves, and combined the resulting features with cyclic Procrustes alignment and product-MMD hypothesis testing. Four EBSD examples spanning Truth Table~\ref{tab:logical_conjunction} and a lithium-ion battery SEM segmentation study illustrate the framework end-to-end.
...
\begin{itemize}
\item Funding: This work was partially support by a National Institute of Standards \& Technology (NIST) Building the Future (BTF) internal funding initiative and the second author is supported in part by funds from the National Science Foundation (NSF) Research Training Group (RTG) grant (DMS-2136228). This work was authored in part by the National Laboratory of the Rockies for the U.S. Department of Energy (DOE), operated under Contract No. DE-AC36-08GO28308. This work was supported by the U.S. Department of Energy Office of Critical Minerals and Energy Innovation. 
...
\bibitem{doronina2025aerodynamic}
O.~A. Doronina, B.~Lee, Z.~J. Grey, A.~Glaws, \emph{Aerodynamic sensitivities over separable shape tensors}, in \emph{AIAA Journal}, vol.~63 (2025), pp. 2707--2720.
\newblock \urlprefix\url{https://arc.aiaa.org/doi/abs/10.2514/1.J064749}
...
\end{itemize}
The polar choice is adopted throughout this paper because it separates undulation from scale variations independent of rotations and reflections: following~\cite{fletcher2003statistics}, $S^d_{++}$ is identified with $GL_+(d,\mathbb{R})/SO(d)$, so that $[\widetilde{X}]$ encodes nonlinear undulating features over $\mathbb{R}^{n \times d}_*/GL(d,\mathbb{R})$ while $P$ encodes generalized (anisotropic) \emph{scales} over $GL_+(d,\mathbb{R})/SO(d)$. This quotient reading is the geometric justification for the separable shape tensor~\eqref{eq:SST} introduced in the main body.

\paragraph{Tangent PCA algorithmic details.}
Section~\ref{subsec:mfld_learn} states the tangent PCA (tPCA) procedure over $\mathcal{G}_r \subseteq Gr(d,n)$ and $\mathcal{P}\subseteq S^d_{++}$ at a high level; we collect here the algorithmic components from~\cite{grey2023separable,fletcher2003statistics,fletcher2004principal,edelman1998geometry,bendokat2020grassmann} that inform the \emph{discretize}$\to$\emph{test} pipeline. Given a sample $\lbrace \boldsymbol{p}_k\rbrace_{k=1}^N$ on a Riemannian manifold (either $Gr(d,n)$ or $S^d_{++}$), the intrinsic (Fr\'echet) mean $\boldsymbol{p}_0$ is computed by the Karcher iteration~\cite{pennec1999probabilities}
\begin{multline}\label{eq:Frechet_iter}
    \boldsymbol{p} \,\leftarrow\, \mathrm{Exp}_{\boldsymbol{p}}\!\left(\tfrac{1}{N}\sum_{k=1}^N \mathrm{Log}_{\boldsymbol{p}}(\boldsymbol{p}_k)\right), \\
    \quad \text{until } \Big\|\tfrac{1}{N}\sum_{k=1}^N \mathrm{Log}_{\boldsymbol{p}}(\boldsymbol{p}_k)\Big\| < \epsilon,
\end{multline}
and the reduced principal directions in $T_{\boldsymbol{p}_0}M$ are obtained from the thin SVD,
\begin{multline}\label{eq:PGA_SVD}
    \sqrt{N-1}T \coloneqq \\
    \big[\,\mathrm{vec}(\mathrm{Log}_{\boldsymbol{p}_0}(\boldsymbol{p}_1)),\,\dots,
    \,\mathrm{vec}(\mathrm{Log}_{\boldsymbol{p}_0}(\boldsymbol{p}_N))\,\big],  
\end{multline}
$T\overset{SVD}{=} U\Sigma V^{\top}$. retaining the first $r$ left singular vectors $U_r$ as an orthonormal basis of $T_{\boldsymbol{p}_0}\mathcal{G}_r$ (or, for $S^d_{++}$, the full three-dimensional span at $P_0$); the columns of $ U_r^{\top}\boldsymbol{N} = \Sigma_r V_r^{\top}$ are the normal coordinates. The map $\mathrm{vec}:\mathbb{R}^{n\times d}\to\mathbb{R}^{nd}$ column-stacks a tangent matrix; since $\mathrm{tr}(A^{\top}B)=\mathrm{vec}(A)^{\top}\mathrm{vec}(B)$, the SVD~\eqref{eq:PGA_SVD} preserves the Frobenius inner product induced on the tangent space, and the reshape $\mathrm{vec}^{-1}$ recovers matrix-valued directions for composition with $\mathrm{Exp}_{\boldsymbol{p}_0}$. Over $S^d_{++}$, $\mathrm{vec}$ stores only the $d(d+1)/2$ unique symmetric entries.

The intrinsic maps on $Gr(d,n)$ are given with Stiefel representatives~\cite{edelman1998geometry,bendokat2020grassmann}: for a direction $\Delta \in T_{[\widetilde{X}]}Gr(d,n)$ with thin SVD $\Delta = \widehat{U}\widehat{\Sigma}\widehat{V}^{\top}$, the Grassmann exponential is
\begin{equation}\label{eq:Gr_Exp}
    \mathrm{Exp}_{[\widetilde{X}]}(\Delta) = \big[\,\widetilde{X}\widehat{V}\cos(\widehat{\Sigma})\widehat{V}^{\top} + \widehat{U}\sin(\widehat{\Sigma})\widehat{V}^{\top}\,\big],
\end{equation}
and, for a second representative $\widetilde{Y}$ with projection $(I_n - \widetilde{X}\widetilde{X}^{\top})\widetilde{Y}(\widetilde{X}^{\top}\widetilde{Y})^{-1} = \bar U \bar\Sigma \bar V^{\top}$, the Grassmann logarithm is
\begin{equation}\label{eq:Gr_Log}
    \mathrm{Log}_{[\widetilde{X}]}([\widetilde{Y}]) = \bar U\,\arctan(\bar\Sigma)\,\bar V^{\top}.
\end{equation}
Each map requires a single thin SVD, so the complexity is $O(nd^2)$---\emph{linear in $n$ for fixed ambient dimension $d$}, a key enabler of the ensemble-scale computations reported in Section~\ref{sec:experiments}. On $S^d_{++}$ with the affine-invariant metric~\cite{fletcher2003statistics,pennec2020manifold}, the corresponding maps reduce to matrix exponentials and logarithms, $\mathrm{Exp}_P(S) = P^{1/2}\exp\!\big(P^{-1/2}SP^{-1/2}\big)P^{1/2}$ and $ \mathrm{Log}_P(D) = P^{1/2}\log\!\big(P^{-1/2}DP^{-1/2}\big)P^{1/2}$.

One caveat of the Grassmann logarithm~\eqref{eq:Gr_Log} is that $\mathrm{Exp}_{[\widetilde{X}]}\!\circ\,\mathrm{Log}_{[\widetilde{X}]}(\widetilde{Y}) = \widehat{Y}$ with $[\widehat{Y}]=[\widetilde{Y}]$ but with a possible rotational mismatch between representatives; a modified logarithm in~\cite{bendokat2020grassmann} (Alg.~5.3) removes this mismatch at the cost of a second SVD. In this work the mismatch is irrelevant for computing normal coordinates but motivates the cyclic Procrustes registration of Section~\ref{subsec:cycl_Pro} when a fixed representative is needed across the ensemble. A product-manifold geodesic $(\mathrm{Exp}_{1,[\widetilde{V}_0]}(t\Delta),\mathrm{Exp}_{2,P_0}(tS))$ on $\mathcal{G}_r\times\mathcal{P}$ then parametrizes the separable shape tensor~\eqref{eq:SST} componentwise, consistent with the Riemannian-product construction~\cite{grey2023separable}.
