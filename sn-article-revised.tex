...
\subsection{Contributions and Technical Outline}
\textcolor{blue}{Our procedure---\emph{decompose} shapes via separable shape tensors, \emph{discretize} and learn feature distributions over matrix manifolds, and \emph{test} for statistically significant discrepancies via product MMD---delivers the following contributions, each pointed at the section that develops it. Results inherited from~\cite{grey2023separable} are labeled \textbf{[prior]}; new contributions are labeled \textbf{[new]}.}

\textcolor{blue}{\begin{itemize}
    \item \textbf{[prior]}~\cite{grey2023separable} SST polar standardization~\eqref{eq:SST} over $Gr(d,n)\times S^d_{++}$ (Section~\ref{subsec:SST}; supplementary derivations in Appendix~\ref{secA0}).
    \item \textbf{[prior]}~\cite{grey2023separable, fletcher2003statistics, fletcher2004principal, edelman1998geometry, bendokat2020grassmann} Separable tangent PCA algorithms for intrinsic means and principal geodesic directions on $Gr(d,n)$ and $S^d_{++}$ (reviewed in Section~\ref{subsec:mfld_learn}; algorithmic details in Appendix~\ref{secA0}).
    \item \textbf{[new, Section~\ref{sec:FIE}]} Interpretation of the composite linear operator (CLO) as a Hilbert-Schmidt integral operator on $\mathcal{C}_d\cap\mathcal{H}_d$, with PRRTI invariance and eigenfunctions given as scaled evaluation functionals of an $\mathbb{R}^d$-valued RKHS (Definitions~\ref{def:CLO}, \ref{def:PRRTI}; Lemmas~\ref{lemma:symm}, \ref{lemma:HS_int_opt}, \ref{lemma:CLO_invariance}; Theorem~\ref{theorem:eigfuncs}).
    ...
    \item \textbf{[new, Section~\ref{subsec:MMD}]} Formulation of two-sample testing as the binary mapping $H:(\mathcal{E},\widehat{\mathcal{E}})\mapsto\{0,1\}$ with De-Morgan/Bonferroni aggregation of marginal MMD tests (Table~\ref{tab:logical_conjunction}).
...
\textcolor{blue}{Our primary theoretical contribution is the second bullet: relating the SST configuration space over finite-dimensional matrix manifolds to a functional analysis over an infinite-dimensional RKHS. This extends the classical configuration-space work of Kendall et al.~\cite{kendall2009shape}---which has no demonstrated connection to an infinite-dimensional space of curves---and permits flexible kernel choice~\cite{micheli2013matrix} to control regularity of a Hilbert space of curves.}

\textcolor{blue}{A primary practical contribution is the framework's computational efficiency, which enables shape-comparison questions that prior approaches cannot practically address. Registration reduces to finite SVDs and cyclic permutations against a single asymmetric archetype (Theorem~\ref{thm:cycl_Procrustes}) rather than an optimization over the space of diffeomorphisms as in elastic and Sobolev-metric curve frameworks~\cite{srivastava2010shape, bauer2017sobolev, hartman2023varigrad}. Thousands of curves can therefore be decomposed and tested in seconds on a standard laptop. This speed is what makes the parameter studies of Section~\ref{sec:experiments} tractable: the decision-landscape diagnostic sweeping over $(r,n)$, the detection of sub-visual smoothing-induced discrepancies (Figure~\ref{fig:compare_grains_010}), and the lithium-ion battery SEM `goldilocks' analysis of how smoothing scales obscure or preserve distinguishing features. To our knowledge, comparisons at this scale and over such parameter sweeps have not been previously demonstrated for curve ensembles.}
...
\end{equation}
with $X\in\mathbb{R}^{n\times d}_*$ full-rank to exclude degenerate points/lines~\cite{absil2008optimization, grey2023separable}. Appendix~\ref{secA0} collects the detailed derivations; here we summarize only what is needed downstream.
...
\textcolor{red}{PLACEHOLDER: OPTIONALLY ADD ILLUSTRATIVE FIGURES: SCHEMATIC CONTRASTING NONLINEAR UNDULATION VS. LINEAR SCALE/ROTATION/SHEAR (EXPLICITLY REQUESTED BY R2). }
...
\begin{proof}
    ...
    \end{align*}
    for some $Q_p =  U_p^{\top}RV_p \in O(d)$ which satisfies $\langle Q_p, \Omega_p\rangle_F \leq \langle \mathbb{I}_d, \Omega_p\rangle_F = \Vert \mathcal{L}(p) \Vert_1$ with equality when $Q_p = \mathbb{I}_d$. However, $Q_p = \mathbb{I}_d$ if and only if $R = U_pV_p^{\top}$ which implies, by $(\star\star)$, $R = \pi_{\mathcal{U}}(\mathcal{L}(p))$. Thus, $\langle R,\mathcal{L}(p)\rangle_F - \Vert \mathcal{L}(p) \Vert_1 < 0$ for all $R \neq \pi_{\mathcal{U}}(\mathcal{L}(p)) \in O(d)$.
\end{proof}
...
This formal problem statement is the quantitative analog of the proposed explainable binary classification: \textit{test if distributions of random shape coordinates partitioned between two images are discrepant, ignoring rigid motions, and whether those discrepancies result from distinct distributions over undulation, $\lbrace \boldsymbol{t} \rbrace$ from the first image versus $\lbrace \widehat{\boldsymbol{t}} \rbrace$ from the second, \emph{or} generalized scale, $\lbrace \boldsymbol{\ell} \rbrace$ from the first image versus $\lbrace \widehat{\boldsymbol{\ell}} \rbrace$ from the second.}
...
\textcolor{blue}{We instantiate the MMD hypothesis test with a non-parametric sample-permutation procedure. The test statistic is the biased empirical estimator $\widehat{\mathrm{MMD}}^2$ from Lemma~6 of~\cite{gretton2012kernel}, computed separately for undulation and scale coordinates. The RKHS $\mathcal{F}$ is taken to be the Gaussian RBF reproducing kernel $\exp(-\Vert\boldsymbol{x}-\boldsymbol{y}\Vert^2/(2\sigma^2))$ with bandwidth $\sigma$ fixed to the median pairwise distance over the pooled sample---the standard `median heuristic'~\cite{gretton2012kernel}.}

\textcolor{blue}{Two distinct permutation operations enter the procedure and must not be conflated. \emph{Cyclic Procrustes permutation} (Theorem~\ref{thm:cycl_Procrustes}) acts on the \emph{rows} of each individual landmark matrix to register every shape against the random asymmetric archetype; it is a row-level registration step performed once \emph{a priori}, yielding pre-registered normal coordinates $(\boldsymbol{t}_q,\boldsymbol{\ell}_q)$. \emph{Sample permutations}, by contrast, act on the \emph{ensemble labels} of these pre-registered coordinates: for each of $B = 1000$ Monte Carlo draws, the $N + \widehat{N}$ coordinate vectors from the pooled ensembles are randomly reassigned to two groups of sizes $N$ and $\widehat{N}$, and $\widehat{\mathrm{MMD}}^2_b$ is recomputed on that partition. The conservative permutation $p$-value is then
\begin{equation}\label{eq:perm_pvalue}
    p \;=\; \frac{1\,+\,\#\{b\in\{1,\dots,B\}\,:\,\widehat{\mathrm{MMD}}^2_b \,\geq\, \widehat{\mathrm{MMD}}^2_{\mathrm{obs}}\}}{1+B},
\end{equation}
and we reject at significance level $\alpha$ whenever $p \leq \alpha$ (with the Bonferroni adjustment above applied to each marginal test). The two permutations are orthogonal: cyclic Procrustes registration is label-agnostic, so the null distribution of the test statistic under exchangeable ensemble labels is preserved by pre-registration.}
...
We begin with a summary investigating material micrographs with EBSD followed by a study involving a more general grayscale imaging modality.
...
\begin{algorithmic}[1]
\color{blue}
\Require Two ensembles of segmented closed curves $\{\gamma_q\}_{q=1}^{N}$ and $\{\widehat{\gamma}_q\}_{q=1}^{\widehat{N}}$; quadrature size $n$; undulation rank $r$; significance level $\alpha$; permutation count $B$.
...
\State \textbf{Sample permutations}: for $b=1,\dots,B$, randomly reassign the pooled ensemble labels and recompute $\widehat{\mathrm{MMD}}^2_b$; form the conservative permutation $p$-values $p_{\boldsymbol{t}}$ and $p_{\boldsymbol{\ell}}$ via~\eqref{eq:perm_pvalue}.
\State \textbf{Bonferroni conjunction}: reject the marginal null at level $\alpha/2$ whenever $p_\bullet \leq \alpha/2$, yielding $H_{\boldsymbol{t}}, H_{\boldsymbol{\ell}}\in\{0,1\}$; the joint decision is $H = H_{\boldsymbol{t}} \wedge H_{\boldsymbol{\ell}}$.
...
\textcolor{blue}{Before examining the decision landscapes, we note that they are themselves the primary diagnostic for selecting the quadrature size $n$ and the undulation rank $r$. Spectral convergence of Nystr\"om quadrature on smooth closed curves means a modest $n$ suffices; Figures~\ref{fig:010_decision} and~\ref{fig:010_decision-normalized} confirm decisions are stable across $n\in[100,500]$, so $n$ should simply be large enough that the landmarks visually resolve the worst-case boundary in the ensemble. The rank $r$ is more consequential but should \emph{not} be chosen to maximize variance coverage: smaller $r$ acts as a dimension reduction that regularizes shapes against noisy undulations (Figure~\ref{fig:low_dim_grain}), which is often desirable. We instead study decision variability directly via the \emph{ambiguity}~\eqref{eq:ambiguity}---a scaled Bernoulli variation of the rejection proportion over $r$---and choose $r$ beyond the ambiguity peak so that decisions are stable against the chosen regularization level.}
