...
\textcolor{blue}{We emphasize that $H$ is a distribution-level binary classifier over pairs of ensembles, not an instance-level label predictor. Two-sample hypothesis testing is a binary classification in the sense of mapping $(\mathcal{E},\widehat{\mathcal{E}})$ to \{reject, fail to reject (FTR)\}. Ensembles are implicitly clustered to the images that generate them---i.e., $\mathcal{E}_i \mapsto i$ is a natural clustering. This allows us to generalize binary classification to \textit{context-driven classification}: fix one argument of $H(\cdot,\cdot)$ to a curated `trusted' ensemble and sweep the other over `discovered' ensembles to label each via its pairwise decision, or enumerate pairwise comparisons across a database of trusted references.}

`Difference' in our geometric context is stochastic and focused on invariance to rigid motions of segmented curves. Randomness arises from aleatoric uncertainty in the imaged objects---e.g., Figure~\ref{fig:eg_grains}---and epistemic uncertainty in segmentation hyperparameters. Because noisy measurements and uncertain segmentation require stochastic decision-making, a naive deterministic approach, such as comparing pointwise pixel differences, is uninformative. No two samples of a random process are identical thus concluding they differ based on single-pixel variations is a useless formalism absent perfect pixel registrations.

Scientists often utilize expertise and visual inspection to suggest how simple characteristics---e.g., principal lengths, area, perimeter, size, sphericity, and other intuitive features---are similar and different between patterns in pairs of images~\cite{stoudt2020location, ASTM_E562, ASTM_E930, ASTM_E112}. While such quantities can provide a cursory description of the ensemble characteristics, these handpicked features of shape do not constitute comprehensive representations of shape; descriptions of differences can collapse in unexpected ways. For example, it is straightforward to find examples of two shapes with identical hand-picked shape characteristics but other distinguishing features. Despite this, for example, certain commercial standards~\cite{kazakov2019astm, ASTM_E562, ASTM_E930, ASTM_E112} for materials quality have been authored to detect hand-picked features. We expect our shape \textit{representations} will offer dramatic improvements in future materials standards but extend well beyond materials science applications.
...
\textcolor{blue}{Our procedure---\emph{decompose} shapes via separable shape tensors, \emph{discretize} and learn feature distributions over matrix manifolds, and \emph{test} for statistically significant discrepancies via product MMD---delivers the following contributions, each pointed at the section that develops it. Results inherited from~\cite{grey2023separable} are labeled \textbf{[prior art]}; new contributions are labeled \textbf{[new]}.}
...
%\textcolor{blue}{We instantiate the MMD hypothesis test with a non-parametric sample-permutation procedure called \textit{bootstrapping}. The test statistic is the biased empirical estimator of~\cite{gretton2012kernel}, computed separately for undulation and scale coordinates. The RKHS $\mathcal{F}$ is taken to be the Gaussian RBF reproducing kernel with parametric length scale assigned to the median pairwise distance over the pooled sample---the standard `median heuristic'~\cite{gretton2012kernel}.}
...
\textcolor{blue}{The sole intrinsically sequential stage is tPCA---Karcher's algorithm to approximate the Fr\'echet mean is iterative and applied over the aggregate ensemble---but Table~\ref{tab:timing} shows tPCA is among the cheapest columns at every $N$, so the single-threaded cost is tolerable. SRQD reparameterization therefore dominates the pipeline and is the highest-leverage optimization target. As an independent demonstration that this acceleration is within reach, an equivalent workflow in MATLAB~R2024b on a $20$-thread Ubuntu workstation (SRQD invoked through \texttt{parfor}) reduces the $N=2000$ SRQD entry to $3.45$\, sec.---an approximately $50\times$ improvement over the single-threaded Python reference. Full mathematical software packages in both MATLAB and Python are forthcoming.}
