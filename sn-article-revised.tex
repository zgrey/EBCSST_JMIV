...
\begin{table}
    ...
    \begin{tabular}{r|rrrr}
        ...
        \hline
        $\widehat{\nu}$ & 1.02 & 1.00 & 1.01 & 2.07 \\
    \end{tabular}
    \caption{Wall-clock timings (seconds) for the four principal pipeline stages as the ensemble size $N$ increases. The final row reports the empirical power-law exponent $\widehat{\nu}$ fit to $t(N) \approx C\, N^{\widehat{\nu}}$ via ordinary least squares on $(\log N,\,\log t)$; values near unity indicate linear scaling in ensemble size.}
    \label{tab:timing}
\end{table}
We report various timings using subsets of curves segmented from the battery data in Table~\ref{tab:timing}. The SRQD column subsumes the arc-length reparameterization it invokes internally. The pMMD two-sample test is evaluated on an even split ($N/2$ versus $N/2$) with $B =1000$ bootstrap permutations. All stages were executed sequentially (single-threaded) to expose clean scaling laws on the algorithm itself. Measurements are from the Python reference implementation at $n=500$ landmarks and $r=50$ reduced dimensions, run on a $16$-core / $32$-thread x86-64 desktop CPU (up to $4.3$\,GHz boost) with $64$\,GB of system memory under Windows~11 and Python~3.13.

A rough partition of the pipeline into \emph{embarrassingly parallel} versus \emph{intrinsically sequential} work makes the headroom for acceleration clear. The per-curve stages---arc-length reparameterization (subsumed by SRQD) and cyclic Procrustes alignment---touch each curve independently and both exhibit essentially linear empirical scaling ($\widehat{\nu}\approx 1$). Because no inter-curve synchronization is required, these stages admit process- or thread-level parallelism with speedups bounded only by worker count and memory bandwidth; the reference implementation exposes this via a pluggable backend switch (sequential, threaded, or multiprocess), with multiprocess preferred for the dominant \texttt{scipy} spline evaluations that otherwise hold the global interpreter lock. The pMMD bootstrap is quadratic in ensemble size ($\widehat{\nu}\approx 2$) because each of the $B=1000$ permutations operates on the pooled $N\times N$ Gaussian kernel matrix, but the permutations themselves are independent and the inner kernel-sum loop is pure index arithmetic that compiles cleanly under a just-in-time compiler. On the test machine, enabling Numba's parallel JIT reduces the $N=2000$ pMMD entry from $30.5$\,s to roughly $0.4$\,s---a two-orders-of-magnitude improvement that effectively removes pMMD from the critical path. The sole sequential bottleneck is tPCA, because computing the Karcher (Fr\'echet) intrinsic mean on the Grassmannian and SPD factors is an iterative reduction---each update is a log/exp step dependent on the previous mean---and the subsequent principal geodesic extraction is a single global singular value decomposition. In practice tPCA remains one of the cheapest columns at every $N$ we tested, so the single-threaded cost is tolerable; the overall pipeline is dominated by SRQD reparameterization, and accelerating that stage is the highest-leverage optimization.
