...
\textcolor{red}{PLACEHOLDER: TECHNICAL DESCRIPTION JUSTIFYING HYPOTHESIS TESTING AS BINARY CLASSIFICATION. CLARIFY THAT TWO-SAMPLE TESTING MAPS PAIRS OF DISTRIBUTIONS TO \{REJECT, FAIL TO REJECT\}, CONSTITUTING A BINARY DECISION RULE. DISTINGUISH FROM SUPERVISED-LEARNING CLASSIFICATION. CONNECT TO THE TITLE: ``EXPLAINABLE BINARY CLASSIFICATION'' = HYPOTHESIS TESTING + INTERPRETABLE SST FEATURES EXPLAINING WHY.}
...
We emphasize that our framework is formulated as \emph{hypothesis testing} which implies a classification rather than clustering. While the learned feature space naturally admits distance-based clustering, hypothesis testing offers a principled binary conclusion---reject or fail to reject---with a controlled significance level, directly answering whether two ensembles are statistically discrepant. That said, clustering over our feature space is a complementary and equally valid approach for exploratory analysis of shape ensembles. \textcolor{red}{PLACEHOLDER: SHOULD WE ADD A NICE EXAMPLE CLUSTERING WITH MORPHOLOGICAL SEGMENTATION OF SUN DATA? PERHAPS A SHORT BLURB ON CLUSTERING COULD BE USEFUL.}

\subsection{Technical Outline}
\textcolor{red}{PLACEHOLDER [ITEM 8, R2]: RESTRUCTURE MANUSCRIPT TO FOREGROUND CONTRIBUTIONS AND MOVE DETAILED BACKGROUND (SECTIONS 2--3) TO SUPPLEMENTARY MATERIAL. CLEARLY DELINEATE WHICH RESULTS ARE FROM PRIOR WORK (G2AERO, GREY 2023) VERSUS NEW TO THIS MANUSCRIPT. R2 FOUND THIS DISTINCTION HARD TO IDENTIFY.}

\textcolor{red}{PLACEHOLDER [ITEM 7, R4]: ADD A PIPELINE ALGORITHM FIGURE (FLOWCHART OR PSEUDOCODE) SUMMARIZING THE FULL METHOD: SEGMENTATION $\to$ LANDMARKS $\to$ SRQD $\to$ CYCLIC PROCRUSTES $\to$ TANGENT PCA $\to$ MMD HYPOTHESIS TESTING. EXPLICITLY REQUESTED BY R4.}
