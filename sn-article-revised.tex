...
\textcolor{blue}{We instantiate the MMD hypothesis test with a non-parametric sample-permutation procedure. The test statistic is the biased empirical estimator $\widehat{\mathrm{MMD}}^2$of~\cite{gretton2012kernel}, computed separately for undulation and scale coordinates. The RKHS $\mathcal{F}$ is taken to be the Gaussian RBF reproducing kernel with scale parameter fixed to the median pairwise distance over the pooled sample---the standard `median heuristic'~\cite{gretton2012kernel}.}

\textcolor{blue}{Two distinct permutation operations enter the procedure and must not be conflated. Cyclic Procrustes permutation (Theorem~\ref{thm:cycl_Procrustes}) acts on the \emph{rows} of each individual landmark matrix to register every shape against the random asymmetric archetype; it is a row-level registration step performed once \emph{a priori}, yielding pre-registered normal coordinates $(\boldsymbol{t}_q,\boldsymbol{\ell}_q)$. Sample permutations, by contrast, act on the \emph{ensemble labels} of these pre-registered coordinates: for each random sample permutation, the $N + \widehat{N}$ coordinate vectors from the pooled ensembles are randomly reassigned to two groups of sizes $N$ and $\widehat{N}$, and $\widehat{\mathrm{MMD}}^2_b$ is recomputed on that partition. We conservatively use $1000$ sample permutations in our examples.
}

\textcolor{blue}{
The conservative permutation $p$-value is then rejected at significance level $\alpha$ whenever it is less than $\alpha/2$ with the Bonferroni adjustment applied. The two permutations are distinct: cyclic Procrustes registration is label-agnostic and fixed against unique asymmetric archetype, so the null distribution of the test statistic under exchangeable ensemble labels is preserved by this pre-registration.}
