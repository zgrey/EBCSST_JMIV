...
\textcolor{blue}{For our problem, information is implicitly clustered by virtue of the images being compared---e.g., $\mathcal{E}_i \mapsto i$. Thus binary decisions can be generalized to classification tasks such that $\mathcal{E}_1$ is built from one class of images (dogs) and $\mathcal{E}_2$ is built from another (cats). Moreover, a classification model would naturally accept a single image as input and return a corresponding label. In our case, a binary decision machine is also capable of the context-driven classification by keeping one argument fixed and the other variable or considering all pairwise comparisons of `trusted' (context-driven) ensembles and `discovered' (unknown) ensembles.}
...
\subsection{Notation}
\textcolor{red}{PLACEHOLDER: ADD ADDITIONAL NOTATION DISCUSSION. ESPECIALLY USE OF SQUARE BRACKETS.}
...
\section{Review of Separable Shape Tensors}\label{secA0}
