...
\textcolor{blue}{We report various timings in Table~\ref{tab:timing} using subsets of curves segmented from the battery data. The SRQD column subsumes an arc-length reparameterization it invokes internally. The pMMD two-sample test is evaluated on an even split ($N/2$ versus $N/2$) with $B =1000$ bootstrap permutations. All stages were executed sequentially (single-threaded) to expose clean scaling laws on the algorithm itself. Measurements are taken using a Python implementation at $n=500$ landmarks and $r=50$ reduced dimensions, run on a $16$-core / $32$-thread x86-64 desktop CPU (up to $4.3$\,GHz boost) with $64$\,GB of system memory under Windows~11 and Python~3.13.}
...
\begin{table}
    ...
    \begin{tabular}{r|rrrr}
        ...
        \hline
         100 & 0.21 &   8.47 & 0.21 &  0.06 \\
         %250 & 0.52 &  21.12 & 0.57 &  0.25 \\
         500 & 1.05 &  41.99 & 1.05 &  0.91 \\
         %750 & 1.58 &  64.32 & 1.72 &  2.01 \\
        1000 & 2.14 &  84.47 & 2.08 &  3.76 \\
        %1500 & 3.54 & 126.60 & 3.18 & 15.45 \\
        2000 & 4.19 & 167.58 & 4.48 & 30.47 \\
        \hline
        $\eta$ & 1.02 & 1.00 & 1.01 & 2.07 \\
    \end{tabular}
    \caption{Wall-clock timings (seconds) for the four principal pipeline stages as the ensemble size $N$ increases. The final row reports the empirical power-law exponent $\eta$ fit to $time(N) \approx C\, N^{\eta}$ via ordinary least squares on $(\log N,\,\log time)$; values near unity indicate linear scaling in ensemble size.}
    \label{tab:timing}
\end{table}

\textcolor{blue}{The per-curve stages (SRQD and cyclic Procrustes) scale linearly ($\eta\approx 1$) and the pMMD bootstrap scales quadratically ($\eta\approx 2$) because each of the $B=1000$ permutations touches the pooled $N\times N$ Gaussian kernel matrix. The per-curve stages and pMMD permutations are embarrassingly parallel. Enabling Numba's parallel JIT drops the $N=2000$ pMMD entry from $30.5$\,s to roughly $0.4$\,s and removes it from the critical path. The only intrinsically sequential stage is tPCA, because Karcher's algorithm to approximate the Fr\'echet mean is iterative and applied over the aggregate ensemble. Table~\ref{tab:timing} shows tPCA is among the cheapest columns at every $N$, so SRQD dominates and is the highest-leverage optimization target.}

\textcolor{blue}{An equivalent workflow in MATLAB~R2024b on a $20$-thread Ubuntu workstation with SRQD invoked through \texttt{parfor} reduces the $N=2000$ SRQD entry to $3.45$\,s, an approximately $50\times$ improvement over the single-threaded Python reference. Likewise, on a modest laptop of $16$ GB system memory running Windows 11, MATLAB R2024b \texttt{parfor} over 8 threads (4 cores/workers, 11th Gen intel i7 3 GHz), the entire workflow of Algorithm~\ref{alg:pipeline} using the total ensemble size $N=2000$ runs in approximately $45$ seconds. Full software packages in MATLAB and Python are forthcoming.}
