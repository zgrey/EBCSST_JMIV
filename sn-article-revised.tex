...
\end{equation}
\textcolor{blue}{with $\boldsymbol{t} \in \mathbb{R}^r$ ($1\leq r < d(n-d)$) and $\boldsymbol{\ell}\in \mathbb{R}^3$ learned by separate tangent PCA~\cite{fletcher2004principal, edelman1998geometry, grey2023separable} over the aggregate ensemble.}
...
Section~\ref{subsec:mfld_learn} states the tangent PCA (tPCA) procedure over $\mathcal{G}_r \subseteq Gr(d,n)$ and $\mathcal{P}\subseteq S^d_{++}$ at a high level; we collect here the algorithmic components from~\cite{grey2023separable,fletcher2003statistics,fletcher2004principal,edelman1998geometry,bendokat2020grassmann} that inform the \emph{discretize}$\to$\emph{test} pipeline. Given a sample $\lbrace p_k\rbrace_{k=1}^N$ on a Riemannian manifold $M$ (either $Gr(d,n)$ or $S^d_{++}$), the intrinsic (Fr\'echet) mean $p_0$ is computed by the Karcher iteration~\cite{pennec1999probabilities}
\begin{equation}\label{eq:Frechet_iter}
    p \,\leftarrow\, \mathrm{Exp}_p\!\left(\tfrac{1}{N}\sum_{k=1}^N \mathrm{Log}_p(p_k)\right), \quad \text{until } \Big\|\tfrac{1}{N}\sum_{k=1}^N \mathrm{Log}_p(p_k)\Big\| < \epsilon,
\end{equation}
and the reduced principal directions in $T_{p_0}M$ are obtained from the thin SVD
\begin{equation}\label{eq:PGA_SVD}
    \boldsymbol{N} \coloneqq \tfrac{1}{\sqrt{N-1}}\big[\,\mathrm{vec}(\mathrm{Log}_{p_0}(p_1)),\,\dots,\,\mathrm{vec}(\mathrm{Log}_{p_0}(p_N))\,\big] \overset{SVD}{=} U\Sigma V^{\top},
\end{equation}
retaining the first $r$ left singular vectors $U_r$ as an orthonormal basis of $T_{p_0}\mathcal{G}_r$ (or, for $S^d_{++}$, the full three-dimensional span at $P_0$); the columns of $T = U_r^{\top}\boldsymbol{N} = \Sigma_r V_r^{\top}$ are the normal coordinates. The map $\mathrm{vec}:\mathbb{R}^{n\times d}\to\mathbb{R}^{nd}$ column-stacks a tangent matrix; since $\mathrm{tr}(A^{\top}B)=\mathrm{vec}(A)^{\top}\mathrm{vec}(B)$, the SVD~\eqref{eq:PGA_SVD} preserves the Frobenius inner product induced on the tangent space, and the reshape $\mathrm{vec}^{-1}$ recovers matrix-valued directions for composition with $\mathrm{Exp}_{p_0}$. Over $S^d_{++}$, $\mathrm{vec}$ stores only the $d(d+1)/2$ unique symmetric entries.}
...
\end{equation}
Each map requires a single thin SVD, so the complexity is $O(nd^2)$---\emph{linear in $n$ for fixed ambient dimension $d$}, a key enabler of the ensemble-scale computations reported in Section~\ref{sec:experiments}. On $S^d_{++}$ with the affine-invariant metric~\cite{fletcher2003statistics,pennec2020manifold}, the corresponding maps reduce to matrix exponentials and logarithms,
\begin{equation}\label{eq:SPD_ExpLog}
    \mathrm{Exp}_P(S) = P^{1/2}\exp\!\big(P^{-1/2}SP^{-1/2}\big)P^{1/2}, \qquad \mathrm{Log}_P(D) = P^{1/2}\log\!\big(P^{-1/2}DP^{-1/2}\big)P^{1/2}.
\end{equation}}
