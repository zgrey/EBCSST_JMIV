...
1. **MMD permutation-test procedure.** (R1.4, R2.3) [D]
   Replace placeholder at line ~911 in §`subsec:MMD` with a blue-text block: unbiased $\widehat{\mathrm{MMD}}^2$ (Gretton et al. Lemma 6), Gaussian RBF with median-distance bandwidth, $B=1000$ label permutations on pre-registered $(\boldsymbol{t}_q,\boldsymbol{\ell}_q)$, and $p=(1+\#\{b:\widehat{\mathrm{MMD}}^2_b\geq\widehat{\mathrm{MMD}}^2_{\mathrm{obs}}\})/(1+B)$. State that cyclic Procrustes is run once on the pooled aggregate before the permutation loop. Mirrors reply R1.4.
...
4. **Prior-vs-new contributions delineation.** (R2.6) [D]
   Replace placeholder at line 198 (§`sec:intro` contributions list) with ~5 blue-text bullets labeling inherited vs new: inherited from `\cite{grey2023sst}` = CLO, PRRTI eigenfunctions, SST polar decomposition; new here = cyclic Procrustes against a random asymmetric archetype, separable tangent PCA, pMMD hypothesis test with De-Morgan/Bonferroni logic, decision-landscape diagnostic. Mirrors reply R2.6.
...
9. **Type I / power simulated-curve study.** (R1.5, R2.3) [N, NF]
...
10. **Per-stage runtime table.** (R1.6, NF) [N]
...
11. **Non-EBSD demonstration.** (R4.3) [D, W]
